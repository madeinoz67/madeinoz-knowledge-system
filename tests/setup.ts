...
export function createMockContainerManager() {
  return {
    ...
    getNetworkInfo: async (
      name: string
    ): Promise<{ name: string; exists: boolean; subnet: string | undefined }> => {
    ...
    stopAndRemoveContainer: async (_name: string): Promise<void> => {
      // Mock implementation
    },
  };
}
  ...
  return {
    ...
    load: async () => {
      ...
      return {
        ...
        EMBEDDER_PROVIDER:
          env.EMBEDDER_PROVIDER || env.MADEINOZ_KNOWLEDGE_EMBEDDER_PROVIDER || 'openai',
    ...
    save: async (config: Record<string, string | undefined>) => {
      ...
      lines.push(
        `MADEINOZ_KNOWLEDGE_GRAPHITI_TELEMETRY_ENABLED=${config.GRAPHITI_TELEMETRY_ENABLED || 'false'}`
      );

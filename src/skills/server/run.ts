...
async function main() {
  ...
  cli.dim('  View logs:     bun run server logs');
  cli.dim('  Stop system:   bun run server stop');
  cli.dim('  Start system:  bun run server start');
  cli.dim('  Restart:       bun run server restart');
  cli.dim('  Check status:  bun run server status');
  cli.blank();
}
